...
function updateSunDirection(simulatedTimeSeconds) {
    ...
    const sunPos_threeJsX = -sunX_eci; // To match Three.js X with ECI X (0 longitude aligns with -X in Three.js world)
...
window.addOrUpdateSatelliteInScene = function(satelliteData) {
    ...
    const initialEpochUTC = satelliteData.epoch ? new Date(satelliteData.epoch).getTime() : window.currentEpochUTC;
    ...
    if (satelliteData.tleLine1 && satelliteData.tleLine2) {
        try {
            const parsedTle = parseTle(satelliteData.tleLine1, satelliteData.tleLine2);
            // Update global epoch to TLE epoch
            window.currentEpochUTC = parsedTle.epochTimestamp;
            window.totalSimulatedTime = 0; // Reset simulation time to align with new epoch
        } catch (error) {
            console.error(`Failed to parse TLE for satellite ${uniqueId}:`, error);
            if (typeof window.showCustomAlert === 'function') {
                window.showCustomAlert(`Invalid TLE data for satellite ${uniqueId}. Using Keplerian parameters.`);
            }
            satelliteData.tleLine1 = null; // Fallback to Keplerian
            satelliteData.tleLine2 = null;
        }
    }
    ...
    if (existingSat) {
        if (satelliteData.tleLine1 && satelliteData.tleLine2) {
            ...
            existingSat.parsedTle = parseTle(satelliteData.tleLine1, satelliteData.tleLine2);
            existingSat.initialEpochUTC = initialEpochUTC;
        } else {
...
window.viewSimulation = function(data) {
    ...
    if (data.epoch && !(data.tleLine1 && data.tleLine2)) {
...
function animate() { // timestamp is passed by requestAnimationFrame
    ...
    earthMesh.material.uniforms.uCameraPosition.value.copy(camera.position);
                // Ensure the glow material receives the correct sun and camera positions
        if (atmosphereGlowMesh && atmosphereGlowMesh.material && atmosphereGlowMesh.material.uniforms) {
    ...
    const core3D = window.getSimulationCoreObjects(); // Get latest state
    if (core3D.isAnimating) {
        ...
        earthGroup.rotation.y = earthRotationAngle; // Earth rotates around its Y-axis (North Pole)
        if (cloudsMesh) {
            // Clouds rotate slightly faster than Earth to simulate atmospheric dynamics
            const cloudsDifferentialRotationSpeed = 0.05 * EARTH_ANGULAR_VELOCITY_RAD_PER_SEC; // Example: 5% faster
            cloudsMesh.rotation.y += cloudsDifferentialRotationSpeed * frameDeltaTime;
        }

        core3D.activeSatellites.forEach(satellite => {
            satellite.updatePosition(core3D.totalSimulatedTime, frameDeltaTime);
        });

        core3D.activeGroundStations.forEach(gs => {
    ...
    // Update the uniforms in the Earth shader
    if (earthMesh && earthMesh.material && earthMesh.material.uniforms) {
        ...
        earthMesh.material.uniforms.uCameraPosition.value.copy(camera.position); // Pass camera's world position
    }


    // Only draw 2D if active, otherwise ensure 3D is rendered
