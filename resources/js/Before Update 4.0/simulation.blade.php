            ...
            if (parentList) {
                ...
                newFileItem.addEventListener('click', function() {
                    ...
                    if (clickedFileType === 'single' || clickedFileType === 'constellation') {
                        ...
                        if (satData) {
                            window.selectOutputItem(clickedFileName, clickedFileType); // Use selectOutputItem for unified handling
...
function selectOutputItem(id, type) {
    ...
    if (type === 'single') {
        ...
        if (window.viewSimulation && selectedData) {
            // Pass the single satellite data to viewSimulation
        ...
        if (window.viewSimulation && selectedData) {
            // Pass the constellation data object to viewSimulation
            window.viewSimulation(selectedData);
        } else if (window.viewSimulation) {
        ...
        if (window.viewSimulation && selectedData) {
            window.viewSimulation(selectedData); // Show only this GS in scene
        } else if (window.viewSimulation) {
            window.viewSimulation(null); // Clear scene if GS not found
        }
        ...
        if (window.viewSimulation && selectedData) {
            window.viewSimulation(selectedData); // Clear existing satellites/GS and display LB related elements
        } else if (window.viewSimulation) {
            window.viewSimulation(null); // Clear scene if LB not found
        }
    }
...
window.onload = function() {
    ...
    if (navigationEntries.length > 0 && navigationEntries[0].type === 'reload') {
        ...
        // Load data from localStorage for other navigation types
 ...
 // --- NEW MENU FUNCTIONS --- 
    ...
    //To do : Add the function to edit the Link Budget
